...
class BotTests(unittest.TestCase):
    def setUp(self):
        ...
        self.env = dict(
            ...
            root=SimpleNamespace(
                ...
                after=self.schedule,
                after_cancel=lambda key: self.jobs.pop(key, None)
            )
        )
        ...
        for start, target in [((100, 300), (800, 300)), ((450, 60), (450, 540)),
            ...
            for kind in ALL_BOTS:
                for seed in range(15):
                    ...
                    p0 = self.env["bot_position"](plan, 0)
                    self.assertAlmostEqual(p0[0], plan["start"][0], delta=1e-3)
                    self.assertAlmostEqual(p0[1], plan["start"][1], delta=1e-3)
                    p1 = self.env["bot_position"](plan, 1.0)
                    self.assertAlmostEqual(p1[0], target[0], delta=1e-3)
                    self.assertAlmostEqual(p1[1], target[1], delta=1e-3)
                    for i in range(101):
                        x, y = self.env["bot_position"](plan, i / 100.0)
    ...
    def test_smart_bots_kinematics(self):
        # Min-Jerk bot must start and end with near-zero step size (smooth bell speed)
        jerk_plan = self.env["make_bot_plan"]("bot_smart_jerk", (100, 300), (800, 300), 906, 606, 42)
        pos = [self.env["bot_position"](jerk_plan, i / 20.0) for i in range(21)]
        dists = [math.dist(a, b) for a, b in zip(pos, pos[1:])]
        self.assertLess(dists[0], dists[10])  # starts slower than middle
        self.assertLess(dists[-1], dists[10])  # ends slower than middle
        peak_idx = dists.index(max(dists))
        self.assertTrue(8 <= peak_idx <= 12)  # peak is near the center

        # Biomechanical bot must peak earlier (asymmetric profile, tau^0.8)
        full_plan = self.env["make_bot_plan"]("bot_smart_full", (100, 300), (800, 300), 906, 606, 42)
        pos_f = [self.env["bot_position"](full_plan, i / 40.0) for i in range(41)]
        dists_f = [math.dist(a, b) for a, b in zip(pos_f, pos_f[1:])]
        peak_idx_f = dists_f.index(max(dists_f))
        # Asymmetric peak occurs around 35-45% of trajectory progress (index 13-18 out of 40)
        self.assertLess(peak_idx_f, 22)
    ...
    def test_human_after_bot_clears_bot_metadata_and_still_cancels(self):
        ...
        self.tick()
        with (Path(self.directory.name) / 'P02.csv').open() as f:
